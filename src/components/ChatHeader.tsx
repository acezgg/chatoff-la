  ...
  return <header className="border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 sticky top-0 z-50">
      <div className="flex items-center justify-between h-14 px-4">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="sm" onClick={onToggleSidebar} className="md:hidden h-8 w-8 p-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {chatTitle && <p className="text-xs text-muted-foreground">{chatTitle}</p>}
            </div>
          </div>
        </div>

        <Button onClick={onNewChat} variant="outline" size="sm" className="hidden md:flex gap-2">
